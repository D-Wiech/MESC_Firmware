...
int Infineon6EDL7141Driver_readConfig(struct _6EDL7141_driver *driver,
		struct _6EDL7141_config *config);
int Infineon6EDL7141Driver_readFaultAndWarningStatus(
		struct _6EDL7141_driver *driver, struct _6EDL7141_stat *status);
int Infineon6EDL7141Driver_writeConfig(struct _6EDL7141_driver *driver,
		struct _6EDL7141_config *config);
int Infineon6EDL7141Driver_readStatus(struct _6EDL7141_driver *driver,
		struct _6EDL7141_stat *status);
int Infineon6EDL7141Driver_setCSAMPgain(struct _6EDL7141_driver *driver,
		struct _6EDL7141_config *config, enum CurrentSenseGain);
int Infineon6EDL7141Driver_setDeadTime(struct _6EDL7141_driver *driver,
		struct _6EDL7141_config *config, uint8_t Deadtime_in_nanoseconds);

int Infineon6EDL7141Driver_init(struct _6EDL7141_driver *driver,
		struct _6EDL7141_stat *status, struct _6EDL7141_config *config) {
	...
	config->supply_cfg = dis_cp_pre << 15 | DTOD_200us << 13 | BF_500kHz << 12
			| _3V3 << 10 | HALF_DVDD << 2 | _12V;
	config->adc_cfg = 0;
	config->pwm_cfg = 0;
	config->sensor_cfg = 10 << 5;
	config->wd_cfg = 0;
	config->wd_cfg2 = 0;
	config->idrive_cfg = GDC_500mA << 12 | GDC_400mA << 8 | GDC_500mA << 4
			| GDC_400mA;
	...
	config->csamp_cfg = _8_Events << 14 | DT_0_us << 12 | 0b0 << 11
			| BT_50_ns << 7 | 0b111 << 4 | ViaRegisterConfig << 3 | _32X;
...
void Infineon6EDL7141Driver_ENABLE(struct _6EDL7141_driver *driver) {
	...
	HAL_GPIO_WritePin(driver->ENABLE_GPIO_PORT, driver->ENABLE_PIN,
			GPIO_PIN_SET);
}

int Infineon6EDL7141Driver_readSPI(struct _6EDL7141_driver *driver,
		uint8_t addr, uint16_t *read_data) {
	...
	usdelay(1);
	rslt = HAL_SPI_Transmit(&driver->SPI_Handle, &addr, address_size,
	BUS_TIMEOUT_6EDL);
	if (rslt != HAL_OK) {
		return 1; // specify the starting register address
	}
	rslt = HAL_SPI_Receive(&driver->SPI_Handle, data_read, data_size,
	BUS_TIMEOUT_6EDL);
	HAL_GPIO_WritePin(driver->GPI_PORT, driver->CS_PIN, GPIO_PIN_SET);
...
int Infineon6EDL7141Driver_writeSPI(struct _6EDL7141_driver *driver,
	...
	HAL_GPIO_WritePin(driver->GPI_PORT, driver->CS_PIN, GPIO_PIN_RESET);
	usdelay(1);
	rslt = HAL_SPI_Transmit(&driver->SPI_Handle, &write_addr, address_size,
	BUS_TIMEOUT_6EDL);
	if (rslt != HAL_OK) {
		return 1; // specify the starting register address
	}
	rslt = HAL_SPI_Transmit(&driver->SPI_Handle, data_write, data_size,
	BUS_TIMEOUT_6EDL);
	HAL_GPIO_WritePin(driver->GPI_PORT, driver->CS_PIN, GPIO_PIN_SET);
	if (rslt != HAL_OK) {
...
int Infineon6EDL7141Driver_readFaultAndWarningStatus(
		struct _6EDL7141_driver *driver, struct _6EDL7141_stat *status) {
	return Infineon6EDL7141Driver_readSPI(driver, FAULT_ST_ADDR,
			&status->FAULT_STAT);
}
...
int Infineon6EDL7141Driver_readStatus(struct _6EDL7141_driver *driver,
	...
	Infineon6EDL7141Driver_readSPI(driver, SUPPLY_ST_ADDR,
			&status->SUPPLY_STAT);
...
int Infineon6EDL7141Driver_readConfig(struct _6EDL7141_driver *driver,
	...
	Infineon6EDL7141Driver_readSPI(driver, FAULTS_CLR_ADDR,
			&config->faults_clr);
	Infineon6EDL7141Driver_readSPI(driver, SUPPLY_CFG_ADDR,
			&config->supply_cfg);
	...
	Infineon6EDL7141Driver_readSPI(driver, SENSOR_CFG_ADDR,
			&config->sensor_cfg);
	...
	Infineon6EDL7141Driver_readSPI(driver, IDRIVE_CFG_ADDR,
			&config->idrive_cfg);
	Infineon6EDL7141Driver_readSPI(driver, IDRIVE_PRE_CFG_ADDR,
			&config->idrive_pre_cfg);
	Infineon6EDL7141Driver_readSPI(driver, TDRIVE_SRC_CFG_ADDR,
			&config->tdrive_src_cfg);
	Infineon6EDL7141Driver_readSPI(driver, TDRIVE_SINK_CFG_ADDR,
			&config->tdrive_sink_cfg);
	...
	Infineon6EDL7141Driver_readSPI(driver, CSAMP_CFG2_ADDR,
			&config->csamp_cfg2);
	rslt = Infineon6EDL7141Driver_readSPI(driver, OTP_PROG_ADDR,
			&config->otp_prog);
	return rslt;
}
...
int Infineon6EDL7141Driver_writeConfig(struct _6EDL7141_driver *driver,
	...
	Infineon6EDL7141Driver_writeSPI(driver, FAULTS_CLR_ADDR,
			config->faults_clr);
	Infineon6EDL7141Driver_writeSPI(driver, SUPPLY_CFG_ADDR,
			config->supply_cfg);
	...
	Infineon6EDL7141Driver_writeSPI(driver, SENSOR_CFG_ADDR,
			config->sensor_cfg);
	...
	Infineon6EDL7141Driver_writeSPI(driver, IDRIVE_CFG_ADDR,
			config->idrive_cfg);
	Infineon6EDL7141Driver_writeSPI(driver, IDRIVE_PRE_CFG_ADDR,
			config->idrive_pre_cfg);
	Infineon6EDL7141Driver_writeSPI(driver, TDRIVE_SRC_CFG_ADDR,
			config->tdrive_src_cfg);
	Infineon6EDL7141Driver_writeSPI(driver, TDRIVE_SINK_CFG_ADDR,
			config->tdrive_sink_cfg);
	...
	rslt = Infineon6EDL7141Driver_writeSPI(driver, CSAMP_CFG2_ADDR,
			config->csamp_cfg2);
...
int Infineon6EDL7141Driver_setCSAMPgain(struct _6EDL7141_driver *driver,
		struct _6EDL7141_config *config, enum CurrentSenseGain gain) {
	int rslt = 0;
	uint16_t csamp_reg_area = 0b111;
	csamp_reg_area = ~csamp_reg_area;
	config->csamp_cfg = (config->csamp_cfg & (csamp_reg_area)) | gain;
	rslt = Infineon6EDL7141Driver_writeSPI(driver, CSAMP_CFG_ADDR,
			config->csamp_cfg);
	return rslt;
}
...
int Infineon6EDL7141Driver_setDeadTime(struct _6EDL7141_driver *driver,
	...
	int rslt = 0;
	uint16_t DT_reg_val = ((uint16_t) Deadtime_in_nanoseconds) << 8
			| Deadtime_in_nanoseconds;
