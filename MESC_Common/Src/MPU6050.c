...
int MPU6050Init(I2C_HandleTypeDef *i2c_handle, uint16_t address,
	...
	uint8_t check = 0;
	HAL_I2C_Mem_Read(MPU_instance->MPU6050_I2C, MPU_instance->MPU6050_address,
			0x75, 1, &check, 1, 10);
	__NOP();
	uint8_t Data = 0;
	HAL_I2C_Mem_Write(MPU_instance->MPU6050_I2C, MPU_instance->MPU6050_address,
			0x6B, 1, &Data, 1, 1000); //PWR_MGMT_1_REG
	Data = 0x07;
	HAL_I2C_Mem_Write(MPU_instance->MPU6050_I2C, MPU_instance->MPU6050_address,
			0x19, 1, &Data, 1, 1000); //SMPLRT_DIV_REG
	Data = 0x00;
	HAL_I2C_Mem_Write(MPU_instance->MPU6050_I2C, MPU_instance->MPU6050_address,
			0x1B, 1, &Data, 1, 1000); //GYRO_CONFIG_REG
	Data = 0x00;
	HAL_I2C_Mem_Write(MPU_instance->MPU6050_I2C, MPU_instance->MPU6050_address,
			0x1C, 1, &Data, 1, 1000); //ACCEL_CONFIG_REG
	if (check == 0x68) {
		return 1;
	}
	return 0;
...
int MPU6050GetData(MPU6050_data_t *MPU_instance) {
	...
	MPU_instance->temp = ((int16_t) (get_data[6] << 8 | get_data[7])) / 340
			+ 36;
