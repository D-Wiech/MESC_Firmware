...
//	if(_motor->pos.set_position<200000){_motor->pos.set_position = _motor->pos.set_position+1500;}
	...
	_motor->pos.error = (float) (int) (_motor->pos.set_position
			- _motor->FOC.PLL_angle);
	_motor->pos.p_error = _motor->pos.Kp * _motor->pos.error;
	_motor->pos.int_error = _motor->pos.int_error
			+ _motor->pos.Ki * _motor->pos.p_error;

	_motor->pos.d_pos = (float) (int) (_motor->FOC.PLL_angle
			- _motor->pos.last_pll_pos);
...
//Apply the PID
	if (fabsf(_motor->pos.error) > _motor->pos.deadzone) {
		_motor->FOC.Idq_prereq.q = _motor->pos.p_error + _motor->pos.int_error
				+ _motor->pos.d_error;
	} else {
